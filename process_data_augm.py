...
def process_image(image, box, distortion, width, length):
    # crop the image and only keep the area that contains the numbers
    image = image.crop(box)
    image = image.filter(distortion)
    plt.imshow(image)
...
def load_picture(folder, im_data, min_num_images):
  ...
  if folder == 'train':
      ...
      # size is len(image_files) - 2, since two of the files are the matlab files
      # and the rest are the images, named with a sequential number
      dataset1 = np.ndarray(shape=(len(image_files)-2, image_width, image_height),
                         dtype=np.float32)
      dataset2 = np.ndarray(shape=(len(image_files)-2, image_width, image_height),
                         dtype=np.float32)
      dataset3 = np.ndarray(shape=(len(image_files)-2, image_width, image_height),
                         dtype=np.float32)
      dataset4 = np.ndarray(shape=(len(image_files)-2, image_width, image_height),
                         dtype=np.float32)
      dataset5 = np.ndarray(shape=(len(image_files)-2, image_width, image_height),
                         dtype=np.float32)
  ...
  for i in range(1, num_images+1):
      try:
          ...
          if folder == 'train':
              # Use various filters to create additional data
              image1 = process_image(image, box, ImageFilter.EMBOSS, image_width, image_height)
              dataset[i-1, :, :] = image1
              image2 = process_image(image, box, ImageFilter.BLUR, image_width, image_height)
              dataset2[i-1, :, :] = image2
              image3 = process_image(image, box, ImageFilter.CONTOUR, image_width, image_height)
              dataset3[i-1, :, :] = image3
              image4 = process_image(image, box, ImageFilter.SMOOTH_MORE, image_width, image_height)
              dataset4[i-1, :, :] = image4
              image5 = process_image(image, box, ImageFilter.SHARPEN, image_width, image_height)
              dataset5[i-1, :, :] = image5
  ...
  # If we are processing the training data, create additional datasets to include the additionally generated images
  if folder == 'train':
      #split the set to 90% training dtatset and 10% validation dataset
      train_dataset = dataset[0:int(0.9*num_images), :, :]
      valid_dataset = dataset[int(0.9*num_images):num_images, :, :]
      train_dataset1 = dataset1[0:int(0.9*num_images), :, :]
      valid_dataset1 = dataset1[int(0.9*num_images):num_images, :, :]
      train_dataset2 = dataset2[0:int(0.9*num_images), :, :]
      valid_dataset2 = dataset2[int(0.9*num_images):num_images, :, :]
      train_dataset3 = dataset3[0:int(0.9*num_images), :, :]
      valid_dataset3 = dataset3[int(0.9*num_images):num_images, :, :]
      train_dataset4 = dataset4[0:int(0.9*num_images), :, :]
      valid_dataset4 = dataset4[int(0.9*num_images):num_images, :, :]
      train_dataset5 = dataset5[0:int(0.9*num_images), :, :]
      valid_dataset5 = dataset5[int(0.9*num_images):num_images, :, :]
      ...
      the_dataset1 = {'train_dataset':train_dataset1, 'valid_dataset':valid_dataset1, 'train_labels':train_labels, 'valid_labels':valid_labels}
      the_dataset2 = {'train_dataset':train_dataset2, 'valid_dataset':valid_dataset2, 'train_labels':train_labels, 'valid_labels':valid_labels}
      the_dataset3 = {'train_dataset':train_dataset3, 'valid_dataset':valid_dataset3, 'train_labels':train_labels, 'valid_labels':valid_labels}
      the_dataset4 = {'train_dataset':train_dataset4, 'valid_dataset':valid_dataset4, 'train_labels':train_labels, 'valid_labels':valid_labels}
      the_dataset5 = {'train_dataset':train_dataset5, 'valid_dataset':valid_dataset5, 'train_labels':train_labels, 'valid_labels':valid_labels}
  elif folder == 'test':
  ...
  # If we are processing the training set, we have performed data augmentation, therefore we return a list of datasets
  if folder == 'train':  
      return [the_dataset, the_dataset1, the_dataset2, the_dataset3, the_dataset4, the_dataset5]
  return the_dataset
...
def maybe_pickle(data_folders, im_data, min_num_images_per_class, force=True):
  ...
  for folder in data_folders:
    ...
    if os.path.exists(set_filename) and not force:
      ...
      try:
        if folder == 'train':
            # The training set consists of 6 datasets, which we pickle separately
            for i in range(0,6):
                set_filename = folder+str(i)+'.pickle'
                with open(set_filename, 'wb') as f:
                    print('pickling ', set_filename)
                    pickle.dump(dataset[i], f, pickle.HIGHEST_PROTOCOL)      
        else:
            with open(set_filename, 'wb') as f:
